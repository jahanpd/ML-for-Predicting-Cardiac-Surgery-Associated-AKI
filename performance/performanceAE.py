...
#
#
# pd.DataFrame(cv_results).to_csv("output/AE.csv")
#
...
# pd.DataFrame(final_results).to_csv('output/performanceAE.csv')
...
pd.DataFrame(cv_results).to_csv("output/AEfull.csv")
...
print(aucs.mean(), briers.mean(), sens.mean(), spec.mean())
final_results = {'auc':aucs,'briers':briers,'sens':sens,'spec':spec}
pd.DataFrame(final_results).to_csv('output/performanceAEfull.csv')
