...
# gsearch.fit(scaler.transform(pre.values), y.values)
# clf = gsearch.best_estimator_
# pd.DataFrame(gsearch.cv_results_).to_csv("output/HF/LR.csv")
#
# output = cross_validate(clf, scaler.transform(pre.values), y.values, scoring=metrics,cv=rkf,  verbose=2,return_train_score=True)
# pd.DataFrame(output).to_csv('output/HF/performanceLR.csv')
#
...
clf = gsearch.best_estimator_
pd.DataFrame(gsearch.cv_results_).to_csv("output/HF/RF.csv")

output = cross_validate(clf, pre.values, y.values, scoring=metrics,cv=rkf,  verbose=2,return_train_score=True)
pd.DataFrame(output).to_csv('output/HF/performanceRF.csv')
...
gsearch.fit(scaler.transform(pre.values), y.values)
clf = gsearch.best_estimator_
pd.DataFrame(gsearch.cv_results_).to_csv("output/HF/DT.csv")

output = cross_validate(clf, pre.values, y.values, scoring=metrics,cv=rkf,  verbose=2,return_train_score=True)
pd.DataFrame(output).to_csv('output/HF/performanceDT.csv')
...
gsearch.fit(pre.values, y.values)
clf = gsearch.best_estimator_
pd.DataFrame(gsearch.cv_results_).to_csv("output/HF/GBM.csv")

output = cross_validate(clf, pre.values, y.values, scoring=metrics,cv=rkf,  verbose=2,return_train_score=True)
pd.DataFrame(output).to_csv('output/HF/performanceXGB.csv')
...
gsearch.fit(pre.values, y.values)
clf = gsearch.best_estimator_
pd.DataFrame(gsearch.cv_results_).to_csv("output/HF/KNN.csv")

output = cross_validate(clf, pre.values, y.values, scoring=metrics, cv=rkf,  verbose=2,return_train_score=True)
pd.DataFrame(output).to_csv('output/HF/performanceKNN.csv')
